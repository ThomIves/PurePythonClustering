...
class KMeans(object):
    ...
    def __group_points_by_centroids__(self, grps, KNN_C, KNN_A):
        for pta in KNN_A:
            ...
            if grps[closest_centroid]['centroid'] == []:
                grps[closest_centroid]['centroid'] = \
    ...
    def __determine_hyper_metric__(self, grps):
        total_number_of_clusters = len(grps)
        number_of_dimensions = len(grps[0]['centroid'])
                
        cnt_loc = [0] * number_of_dimensions
        for i in range(number_of_dimensions):
            for j in range(total_number_of_clusters):
                cnt_loc[i] += grps[j]['centroid'][i]
            cnt_loc[i] /= total_number_of_clusters
        ...
        for i in range(len(grps)):
            dist = self.__get_distance_between_two_points__(
                cnt_loc,
                grps[i]['centroid'])
            hyper_centroid += len(grps[i]['points'])*(dist) ** 2

        return hyper_centroid
    ...
    def determine_k_clusters(self, KNN_A):
        ...
        self.hyper_metric_ = self.__determine_hyper_metric__(grps_best)
    ...
    def plot_clusters(self, grps):
        ...
        for i in range(len(grps)):
            Xc.append(grps[i]['centroid'][0])
            Yc.append(grps[i]['centroid'][1])
...
seeds = [[3,3], [10,3], [17,3], [3,10], [10,10], [17,10]]#, [3,17]]
half_range = 2
...
for i in range(2,10,1):
    ...
    grps = kmeans.determine_k_clusters(KNN_A)
    print('Inertia of {} = {}. HM = {}.'.format(
            i, 
            kmeans.inertia_, 
            kmeans.hyper_metric_))
    print('\t', kmeans.inertia_ / kmeans.hyper_metric_ * (factor)**2)
