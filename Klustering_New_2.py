...
class Clusters_AI:
    def __init__(self, pnts):  # , max_strtch=False):
        self.pnts = pnts
        # self.max_strtch = max_strtch
    ...
    def get_mins_and_maxs(self, indices):
        ...
        #             self.pnts[i1], self.pnts[i2])
        #         if dist > maxD:
        #             maxD = dist

        return mins, mids, maxs  # , maxD
    ...
    def find_radius_for_each_group(self):
        ...
        for indices_grp in self.indices_grps:
            ...
            # total = 0.0
            # for i1 in indices_grp:
            #     for i2 in indices_grp:
            #         if i1 == i2:
            #             continue
            #         pt1 = self.pnts[i1]
            #         pt2 = self.pnts[i2]
            #         dist = self.get_distance_between_two_points(pt1, pt2)
            #         total += dist

            # radius = total / len(indices_grp)

            self.radii_D[radius] = indices_grp

        # for indice_grp in self.indices_grps:
        #     mins, mids, maxs = self.get_mins_and_maxs(indice_grp)
        #     radius_to_min = self.get_distance_between_two_points(mins, mids)
        #     radius_to_max = self.get_distance_between_two_points(maxs, mids)
        #     radius = max(radius_to_min, radius_to_max)
        #     self.radii_D[radius] = {
        #         'indices': indice_grp,
        #         'mids': mids}
    ...
    def grow_groups_within_radii(self):
        # For each group, compare mids to pt distance to radius
        # If distance is less than radius, append point to group
        # Then perform a regrouping of indices

        # max_strtch = 0.0
        # if self.max_strtch:
        #     for radius in self.radii_list:
        #         strtch = self.radii_D[radius]['strtch']
        #         if strtch > max_strtch:
        #             max_strtch = strtch
        ...
        for radius in self.radii_list:
            ...
            for grp_index in self.radii_D[radius]:  # ['indices']:
                for pnt_index in range(self.num_pnts):
                    ...
                    if distance <= radius:
                        if pnt_index not in self.radii_D[radius]:
                            ...
                            # self.radii_D[radius]['indices'].append(pnt_index)
...
cAI = Clusters_AI(pnts)  # , max_strtch=max_stretch)
